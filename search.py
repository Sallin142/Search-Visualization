...
def breadth_first_graph_search(problem):
    ...
    while frontier:
        ...
        explored.add(tuple(node.state))  # Convert to tuple for hashability
        for child in node.expand(problem):
            child_state_tuple = tuple(child.state)  # Convert to tuple for hashability
...
def best_first_graph_search(problem, f=None):
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            child_state_tuple = tuple(child.state)  # Convert to tuple for hashability
            if child_state_tuple not in explored and child not in (item[1] for item in frontier.heap):
                frontier.append(child)
            elif child in (item[1] for item in frontier.heap):
                for item in frontier.heap:
                    if item[1] == child:
                        incumbent = item[1]
                        break
                if f(child) < f(incumbent):
                    frontier.heap.remove((f(incumbent), incumbent))
