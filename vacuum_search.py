...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, c, state1, action, state2):
        ...
        if (self.env.costFunc == costFunctions[0]):
            ...
            cost = cost +0.5 * self.computeTurnCost(c.action, action)
        elif (self.env.costFunc == costFunctions[2]):
            cost = 0.5 ** (wid - state2[0])
        else:
            cost = 0.5 ** state2[1]
            
        return cost

    def computeTurnCost(self, current_action, next_action):
        """Calculate the cost of turning from current_action to next_action."""
        turn_cost = 0
        print("current_action: ", current_action, "next_action: ", next_action)
        if current_action == 'UP':
            if next_action == 'LEFT' or next_action == 'RIGHT':
                turn_cost = 0.5
            ...
            if next_action == 'UP' or next_action == 'DOWN':
                turn_cost = 0.5
    ...
    def findMinManhattanDist(self, pos):
        ...
        for dirtyRoom in self.env.dirtyRooms:
            dist = distance_manhattan(pos, dirtyRoom)
            if dist < minDist:
                minDist = dist
        return minDist
    ...
    def findMinEuclidDist(self, pos):
        """find min Euclidean squared distance to any of the dirty rooms."""
        minDist = math.inf
        for dirtyRoom in self.env.dirtyRooms:
            dist = distance_squared(pos, dirtyRoom)
            if dist < minDist:
                minDist = dist
        return minDist
